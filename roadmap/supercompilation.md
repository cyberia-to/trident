...
The endgame is a system that learns from every computation ever run. When a program runs with supercompilation and produces a closed-form result, two things happen: [[zheng]] generates a proof that the closed form is correct for the given inputs, and [[cybergraph]] records `(formula_particle, object_particle) → (result_particle, proof_particle)` as a permanent cyberlink. The next time any neuron on the network calls `ask(ν, object, formula)`, [[nox]] checks this link first. If the answer is there, it costs zero compute — just a [[soft3]] `query()` call that retrieves the result and its proof.
