...
The [[cybergraph]] stores the bundle's [[hemera]] particle; the proof is a cyberlink from the source particle to the compiled particle. Any node in the network can call `ask(verify, package_particle)` and receive the cached verification result — one proof generated by the author, infinite zero-cost verifications for consumers. This is the flywheel: [[Atlas]] accumulates proven packages, the [[cybergraph]] memoizes their verifications, and the cost of trust collapses to sub-millisecond.
