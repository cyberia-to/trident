...
`ProgramBundle` is the distribution format (`reference/ir.md`). The bundle's [[zheng]] proof is verified by any [[warrior-cyber]] instance before the program runs — warrior-cyber calls the zheng verifier on the embedded proof, checks the [[particle]] against the [[cybergraph]], and only then executes. The `.warrior` file is the physical carrier; the [[cybergraph]] is the canonical registry. [[Atlas]] is the on-chain index that maps names to [[particles]].
