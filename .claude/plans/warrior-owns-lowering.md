...
| S1 trisha builds — **MOSTLY DONE** (trident#34, trisha#2, 2026-09-10) | Four breakages found, three fixed: API drift (poseidon2→hemera, `reads_state`), `triton` was not a default feature (that, not stale source, is why the installed binary had no warrior CLI), a `cfg` on a non-existent `trisha-rs` feature, and the serde split — root cause `cubecl-ir` (burn, via trident-lang) enabling `serde/rc` under resolver 2. Fixed by `resolver = "1"` in trisha + trident's new default-on `neural` feature with `default-features = false` on the warrior side. Dead code removed; zero warnings outside `.vendor/` | ½–1 (spent ~1) | `cargo build/test --workspace` green from a clean vendor rebuild; `trisha run --tasm` executes trident-built TASM (12 cycles); `trident bench --skip-neural` green (42 modules, Tri/Hand 1.33x). **Open:** `cargo build --release` still fails on the vendored chain (dev works) — ruled out stale artifacts, the serde split and generated-constraint drift; trisha#1 stays open, install is `--debug`, so the harness still anchors nox against the Rust references |
| S2 contract | `trident::front::compile_to_tir` + `reference/warrior-api.md`; `neural::Target` trait with the Triton impl still in-tree (the `neural` feature gate itself landed early, in S1); `compile_with_options` stops at TIR for stack targets | 1 | trisha builds against the API without touching lowering yet; neural bench numbers unchanged |
