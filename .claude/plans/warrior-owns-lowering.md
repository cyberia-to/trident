...
| S3 the move — **STARTED** (trident#39, trisha#4, 2026-09-10) | Done: the seam. `trident::build_tir_modules` gives per-module TIR (boundaries intact — a flat op list cannot be linked), `tir::linker` is public, and `trisha build` lowers and links on the warrior side, byte-identical to `trident build --target triton` (pinned by trisha's `rs/tests/build_matches_trident.rs`). Remaining: delete trident's copy of `lower/triton.rs` + `linker.rs`, move ~110 TASM-asserting tests (`api/tests/{compile,features,prove,neptune}`, `ir/tir/lower/tests`, `deploy/tests`) to trisha, make `trident build --target triton` delegate, decide `trident::compile`'s default target, then cost model + stack_verifier + neural target + baselines + os/neptune | 2–3 (spent ~½) | `trisha build` end to end on its own output: 229 bytes identical to the compiler's, `run` → 16 for input 5, `prove` 266 ms, `verify` PASS. **Differential harness live again** and immediately found a real compiler bug — `dup 24` emitted for std::crypto::sha256, outside Triton's 0-15 stack range, so that row's assembly never could execute (trident#40) |
