...
|---|---|---|---|
| S0 attic | `silicon/` crate; mir2nox joins `src/import/`; delete kir/lir/gpu + 4 deps; `neural` feature; README "28 backends" section rewritten as sketch crate; `trident compile` off the main binary | 1 | nox path byte-identical: `tests/nox_surface`, `differential`; fresh `cargo install` smoke |
...
| S2 contract | `trident::front::compile_to_tir` + `reference/warrior-api.md`; `neural::Target` trait with the Triton impl still in-tree; `compile_with_options` stops at TIR for stack targets | 1 | trisha builds against the API without touching lowering yet; neural bench numbers unchanged |
