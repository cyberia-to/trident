...
pub struct DecoderConfig {
    /// Number of tokens supplied by the target vocabulary, including EOS.
...
pub struct StackAwareDecoder<B: Backend> {
    /// Token embedding: self.vocab_size → d_model
    ...
    /// Output projection: d_model → self.vocab_size
...
impl DecoderConfig {
    ...
    pub fn init<B: Backend>(&self, device: &B::Device) -> StackAwareDecoder<B> {
        ...
        StackAwareDecoder {
            token_embed: EmbeddingConfig::new(self.vocab_size, self.d_model).init(device),
            ...
            output_proj: LinearConfig::new(self.d_model, self.vocab_size).init(device),
        }
    }
}
...
impl<B: Backend> StackAwareDecoder<B> {
    ...
    ///
    /// Returns: [batch, seq, self.vocab_size] — logits over vocabulary
    pub fn forward(
        &self,
        ...
        self.output_proj.forward(x) // [batch, seq, self.vocab_size]
    }
