...
/// File will be at `{directory}/{tag}.mpk`.
...
pub fn available_checkpoints(directory: &Path) -> Vec<(CheckpointTag, PathBuf)> {
    let dir = directory;
...
pub fn promote_to_production(directory: &Path, source: CheckpointTag) -> Result<(), String> {
    let dir = directory;
