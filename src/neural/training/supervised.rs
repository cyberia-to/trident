...
//! model (GNN encoder + Transformer decoder) on (TirGraph, target assembly) pairs.
...
pub fn train_epoch<B: burn::tensor::backend::AutodiffBackend>(
    target: &dyn crate::neural::Target,
    ...
    for pair in pairs {
        ...
        let state = precompute_sequence_state(target, tokens, 0);
