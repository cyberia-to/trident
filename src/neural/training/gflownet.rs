...
//! to explore the space of valid target assembly programs. The reward signal comes
...
/// Reward for a generated target assembly sequence.
///
/// R(candidate) = epsilon                                     if !valid
///         = 1 + max(0, (baseline_cost - candidate_cost)      if valid
///                      / baseline_cost)
pub fn compute_reward(
    valid: bool,
    candidate_cost: Option<u64>,
    baseline_cost: u64,
    epsilon: f32,
) -> f32 {
    if !valid || candidate_cost.is_none() {
        return epsilon;
    }
    let mc = candidate_cost.unwrap() as f32;
    let cc = baseline_cost as f32;
    if cc <= 0.0 {
        return 1.0;
    }
...
pub fn sample_sequence<B: Backend>(
    ...
    let (tokens, probability, violation) =
        sample_sequence_tensor(target, model, graph, tau, config, device);
    (
        tokens,
        probability.into_data().to_vec::<f32>().unwrap()[0],
        violation,
    )
}

/// Keep selected token probabilities on the autodiff graph. Sampling the
/// token itself is discrete, but its log probability must train the policy.
fn sample_sequence_tensor<B: Backend>(
    target: &dyn Target,
    model: &NeuralCompilerV2<B>,
    graph: &crate::neural::data::tir_graph::TirGraph,
    tau: f32,
    config: &GFlowNetConfig,
    device: &B::Device,
) -> (Vec<u32>, Tensor<B, 1>, Option<usize>) {
    ...
    let vocab_size = target.vocabulary().size();
    let initial_depth = 0i32; // must match training
    let mut tokens = Vec::new();
    let mut log_pf = Tensor::<B, 1>::zeros([1], device);
    ...
    for step in 0..config.max_seq_len {
        ...
        let mut sm_replay = target.grammar(initial_depth);
        ...
        let mut sm_replay2 = target.grammar(initial_depth);
        ...
        if first_violation.is_none() && token != 0 {
            let mut sm_check = target.grammar(initial_depth);
            for &t in &tokens {
                sm_check.step(t);
            }
        ...
        let log_probs = burn::tensor::activation::log_softmax(logits_1d / tau, 0);
        log_pf = log_pf + log_probs.slice([token as usize..token as usize + 1]);
...
pub fn gflownet_step<B: burn::tensor::backend::AutodiffBackend>(
    ...
    let (tokens, log_pf, first_violation) =
        sample_sequence_tensor(target, model, graph, tau, config, device);

    // Decode and validate
    let assembly = target.vocabulary().decode_sequence(&tokens);
    let valid = if assembly.is_empty() {
        false
    } else {
        target.verify_block(baseline, &assembly, 42)
    };

    let candidate_cost = if valid {
        Some(target.cost(&assembly))
    } else {
        None
    };
        ...
        && (step as f32 / config.shaping_steps as f32) < config.shaping_validity_threshold
    {
        ...
        compute_reward(valid, candidate_cost, baseline_cost, config.reward_epsilon)
    };
    ...
    let loss = residual.clone() * residual;
