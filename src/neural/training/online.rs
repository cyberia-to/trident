...
impl PhaseBMetrics {
    ...
    pub fn all_targets_met(&self) -> bool {
        self.validity_rate >= 0.95
            ...
            && self.median_cost_reduction >= 0.10
...
pub fn compute_phase_b_metrics(buffer: &ReplayBuffer) -> PhaseBMetrics {
    ...
    let improved_count = valid_entries
        .iter()
        .filter(|r| r.cost.map(|c| c < r.baseline_cost).unwrap_or(false))
        .count();
        ...
        .filter_map(|r| {
            r.cost.map(|c| {
                if r.baseline_cost == 0 {
                    ...
                    (r.baseline_cost as f32 - c as f32) / r.baseline_cost as f32
                }
            })
        })
        .collect();
    reductions.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
    let median_cost_reduction = if reductions.is_empty() {
        0.0
    } else {
...
mod tests {
    ...
    fn make_result(valid: bool, cycles: Option<u64>, compiler: u64) -> BuildResult {
        BuildResult {
            tir_hash: [0u8; 32],
            generated_assembly: vec!["push 1".into()],
            valid,
            cost: cycles,
            baseline_cost: compiler,
            fallback_used: false,
            timestamp: 0,
            model_version: 1,
        }
    ...
    fn phase_b_metrics_computation() {
        ...
        assert!(metrics.median_cost_reduction > 0.0);
