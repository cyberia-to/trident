...
pub struct BuildResult {
    ...
    pub tir_hash: [u8; 32],
    /// Generated target assembly instructions.
    pub generated_assembly: Vec<String>,
...
impl ReplayBuffer {
    ...
    fn compute_priority(result: &BuildResult) -> f64 {
        ...
        let improvement = result
            .baseline_cost
            .saturating_sub(result.cost.unwrap_or(result.baseline_cost));
        if result.baseline_cost == 0 {
            return 1.0;
        }
        1.0 + (improvement as f64 / result.baseline_cost as f64)
    }
